...
Casco Tauri 2 do **Fredy** (gestor de pedidos). Abre a lista de empresas do Gestor em `/pedidos/empresas?gestor`. Sem regra de negócio. Identifica-se com User-Agent `Fredy/` — o Gestor web no Chrome não muda.

Bolha flutuante (janela sem barra, sempre no topo, fora da taskbar, ícone do Fredy): só aparece ao minimizar. Clique restaura a janela. Arrasta para outro sítio. Não é overlay do Windows — some no Win+D. Com a aba WhatsApp aberta, o WebView nativo fica escondido enquanto a principal está minimizada, para não cobrir a bolha.
...
O protocolo `gestor-pedidos://open` abre o Fredy. Na primeira execução o Windows associa o protocolo a este exe.
...
URL padrão: `{JIFFY_FLOW_R2_PUBLIC_BASE}/stable/update-manifest.stable.json`
...
Setup NSIS **só do Fredy**. Não inclui o Jiffy Print.

```powershell
.\scripts\package-flow.ps1 -GestorUrl "https://app.jiffy.run"
```

Sai em `dist\`. Per-user, atalho **Fredy**, arranque com o Windows. A URL do Gestor fica gravada no `.exe`. No fim do Setup pergunta se já tem o Print; se não, abre o download.
...
brand/logo.png
brand/icon.png
stable/FredySetup.exe
stable/update-manifest.stable.json
releases/0.1.1/Fredy.exe
```
...
2. SHA-256 do `dist\Fredy.exe` (o script imprime).
