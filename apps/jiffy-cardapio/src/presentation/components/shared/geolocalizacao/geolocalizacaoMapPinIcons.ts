...
function criarOpcoesIconePin(cor: string): google.maps.Icon | undefined {
  if (typeof google === 'undefined') return undefined

  const url = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(montarSvgPin(cor))}`
...
export function criarOpcoesIconePinPreferencia(): google.maps.Icon | undefined {
  return criarOpcoesIconePin('#2563eb')
}

export function criarOpcoesIconePinEndereco(): google.maps.Icon | undefined {
  return criarOpcoesIconePin('#171717')
}
