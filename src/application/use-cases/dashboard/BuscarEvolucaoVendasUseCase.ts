...
export class BuscarEvolucaoVendasUseCase {
  ...
  async execute(
    ...
    if (isCustomDates && periodoInicialCustom && periodoFinalCustom) {
      const diffInHours =
        (periodoFinalCustom.getTime() - periodoInicialCustom.getTime()) / (1000 * 60 * 60)
      const hasSpecificHours =
        periodoInicialCustom.getHours() !== 0 ||
        periodoInicialCustom.getMinutes() !== 0 ||
        periodoFinalCustom.getHours() !== 23 ||
        periodoFinalCustom.getMinutes() !== 59
