...
import {
  StatusFiscalVenda,
  primeiroStatusFiscalNaoVazio,
} from '@/src/domain/value-objects/StatusFiscalVenda'
...
/** Etapa logística canônica: `statusDelivery` (API delivery) ou `statusEtapaOperacional` (unificado). */
...
export type EtapaKanbanBalcao = ColunaKanbanFiscalId
...
export class VendaUnificadaDTO {
  constructor(
    ...
    public readonly statusFiscal: StatusFiscalVendaValor | null,
  ...
  isPendenteEmissao(): boolean {
    ...
    if (StatusFiscalVenda.tryParse(this.statusFiscal)?.isInutilizada()) return false

    if (this.isVendaGestor()) {
      return !!this.solicitarEmissaoFiscal && !StatusFiscalVenda.tryParse(this.statusFiscal)?.isEmitida()
    }
  ...
  temNFeEmitida(): boolean {
    ...
    if (fiscal?.temDocumentoAutorizadoOuCancelado() && this.possuiDocumentoFiscal()) return true
  ...
  /**
   * Delivery em curso: coluna operacional.
   * Terminal ou etapa ausente: não inventa NOVOS_PEDIDOS — segue a máquina fiscal.
   */
  private resolverEtapaKanbanOperacionalEntrega(): string | null {
    const etapa = EtapaOperacionalDelivery.tryParse(this.statusEtapaOperacional)
    if (etapa?.aindaOperacional()) {
      return etapa.colunaKanbanOperacional()
    }
    return null
  }
