...
/** True se a opção selecionada está liberada pela disponibilidade da loja. */
export function isOpcaoEntregaDisponivel(params: {
  modoTempo: 'imediato' | 'agendado' | ''
  permiteImediato?: boolean
  aceitaAgendamento?: boolean
  isError?: boolean
}): boolean {
  const { modoTempo, permiteImediato, aceitaAgendamento, isError } = params
  if (!modoTempo || isError) return false
  if (modoTempo === 'imediato') return permiteImediato === true
  return aceitaAgendamento === true
}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {OPCOES.map(({ key, label, tipoEntrega: tipo, modoTempo: modo, Icon }) => {
            ...
            const query = tipo === 'entrega' ? disponibilidadeEntrega : disponibilidadeRetirada
            const disabled =
              query.isLoading ||
              query.isError ||
              (modo === 'imediato'
