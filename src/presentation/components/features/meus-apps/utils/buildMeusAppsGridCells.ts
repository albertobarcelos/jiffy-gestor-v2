 ...
 * Com `expandido: false`, mostra apenas o necessário para a primeira linha (2 itens reais + promo).
 */
export function buildMeusAppsGridCells(
  feedItems: MeusAppsFeedItem[],
  options: { expandido: boolean }
): MeusAppsGridCell[] {
  const visiveis = options.expandido ? feedItems : feedItems.slice(0, 2)
