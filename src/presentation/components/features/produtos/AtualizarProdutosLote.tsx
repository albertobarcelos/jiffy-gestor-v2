...
export function AtualizarPrecoLote() {
  ...
  const aplicarFiscalEmLote = async () => {
    ...
    try {
      ...
      // Optimistic: aplica campos do painel nas linhas com sucesso.
      if (idsFiscalComSucesso.length > 0) {
        const sucessoSet = new Set(idsFiscalComSucesso)
        const ncmOk = ncmTrimmed.length === 8 ? ncmTrimmed : undefined
        const cestOk = cestTrimmed.length === 7 ? cestTrimmed : undefined
        const origemOk = fiscalLoteDraft.origemMercadoria.trim() || undefined
        const tipoOk = fiscalLoteDraft.tipoProduto.trim() || undefined
        const indicadorOk = indTrimmed || undefined

        setProdutos(prev =>
          prev.map(p => {
            if (!sucessoSet.has(p.getId())) return p
            return p.withDadosFiscais({
              ncm: ncmOk ?? p.getNcm(),
              cest: cestOk ?? p.getCest(),
              origemMercadoria: origemOk ?? p.getOrigemMercadoria(),
              tipoProduto: tipoOk ?? p.getTipoProduto(),
              indicadorProducaoEscala:
                indicadorOk !== undefined ? indicadorOk : p.getIndicadorProducaoEscala(),
            })
          })
        )
      }
      ...
      // Re-sincroniza drafts com o resultado do refetch (não usa produtosRef — ainda pode estar stale).
      if (idsFiscalComSucesso.length > 0) {
        const sucessoSet = new Set(idsFiscalComSucesso)
        const porId = new Map(produtosAtualizados.map(p => [p.getId(), p]))
        setFiscalLinhaDrafts(prev => {
          const next = { ...prev }
          for (const id of sucessoSet) {
            const produto = porId.get(id)
            if (produto) next[id] = fiscalLinhaDraftFromProduto(produto)
            else delete next[id]
          }
          return next
        })
  ...
  const fiscalLoteBodyPronto = montarBodyFiscalLote(fiscalLoteDraft) != null
