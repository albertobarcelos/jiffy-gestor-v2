...
interface ImpressoraActionsMenuProps {
  impressoraId: string
  impressoraAtiva: boolean
  onStatusChanged?: () => void
}

/**
 * Menu de ações da impressora
 * Replica exatamente o design do Flutter
 */
export function ImpressoraActionsMenu({
  impressoraId,
  impressoraAtiva,
  onStatusChanged,
}: ImpressoraActionsMenuProps) {
  ...
  const handleEdit = () => {
    setIsOpen(false)
    window.location.href = `/cadastros/impressoras/${impressoraId}/editar`
  }
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      const response = await fetch(`/api/impressoras/${impressoraId}`, {
        ...
        body: JSON.stringify({ ativo: !impressoraAtiva }),
      })
      ...
      alert(
        impressoraAtiva
          ? 'Impressora desativada com sucesso!'
          : 'Impressora ativada com sucesso!'
      )
    } catch (error) {
      console.error('Erro ao atualizar status:', error)
      alert('Erro ao atualizar status da impressora')
    }
  }
    ...
    <div className="relative">
      ...
      {isOpen && (
        ...
        >
          ...
          >
            <span>{impressoraAtiva ? '🔴' : '✅'}</span>
            <span className="text-sm font-medium">
              {impressoraAtiva ? 'Desativar' : 'Ativar'}
            </span>
          </button>
