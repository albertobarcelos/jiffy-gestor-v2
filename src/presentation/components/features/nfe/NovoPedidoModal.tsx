...
export function NovoPedidoModal({
  ...
  const catalogoProdutosPorIdRef = useRef<Record<string, Produto>>({})
  catalogoProdutosPorIdRef.current = catalogoProdutosPorId
  const produtosListRef = useRef<Produto[]>([])
  produtosListRef.current = produtosList

  useEffect(() => {
    const list = produtosPorGrupoData?.produtos
    if (!list?.length) return
    setCatalogoProdutosPorId(prev => {
      const next = { ...prev }
      for (const p of list) {
        if (p.isAtivo()) next[p.getId()] = p
      }
      return next
    })
  }, [produtosPorGrupoData])
  ...
  const carregarProdutoNoCatalogoSeNecessario = useCallback(
    async (produtoId: string): Promise<Produto | null> => {
      const cached =
        ...
        produtosListRef.current.find(p => p.getId() === produtoId)
      if (cached) return cached

      const token = useAuthStore.getState().auth?.getAccessToken()
      if (!token) {
        showToast.error('Sessão indisponível para carregar o produto.')
        return null
      }
      try {
        ...
        if (!res.ok) {
          ...
          showToast.error(err.message || 'Não foi possível carregar o produto.')
          return null
        }
        const data = await res.json()
        const p = Produto.fromJSON(data)
        setCatalogoProdutosPorId(prev => ({ ...prev, [p.getId()]: p }))
        return p
      } catch {
        showToast.error('Não foi possível carregar o produto.')
        return null
      }
    },
    []
