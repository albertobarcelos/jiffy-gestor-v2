...
interface ComplementoActionsMenuProps {
  complementoId: string
  complementoAtivo: boolean
  onStatusChanged?: () => void
}

/**
 * Menu de ações do complemento
 * Replica exatamente o design do Flutter MenuAcaoComplementoWidget
 */
export function ComplementoActionsMenu({
  complementoId,
  complementoAtivo,
  onStatusChanged,
}: ComplementoActionsMenuProps) {
  ...
  const handleEdit = () => {
    setIsOpen(false)
    window.location.href = `/cadastros/complementos/${complementoId}/editar`
  }

  const handleDelete = async () => {
    if (!confirm('Tem certeza que deseja deletar este complemento?')) {
      setIsOpen(false)
      return
    }
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      const response = await fetch(`/api/complementos/${complementoId}`, {
        ...
        body: JSON.stringify({ ativo: !complementoAtivo }),
      })
      ...
      alert(
        complementoAtivo
          ? 'Complemento desativado com sucesso!'
    ...
    <div className="relative">
      ...
      {isOpen && (
        ...
        >
          ...
          >
            <span>{complementoAtivo ? '🔴' : '✅'}</span>
            <span className="text-sm font-medium">
              {complementoAtivo ? 'Desativar' : 'Ativar'}
            </span>
          </button>
