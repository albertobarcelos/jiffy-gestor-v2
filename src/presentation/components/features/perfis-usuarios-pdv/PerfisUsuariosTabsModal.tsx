...
import { NovoUsuario, type NovoUsuarioHandle } from '@/src/presentation/components/features/usuarios/NovoUsuario'
import {
  NovoPerfilUsuario,
  type NovoPerfilUsuarioHandle,
} from './NovoPerfilUsuario'
...
export function PerfisUsuariosTabsModal({
  ...
  const perfilRef = useRef<NovoPerfilUsuarioHandle>(null)
  ...
  /** Alterações no perfil ainda não salvas (baseline) — exige Salvar antes da aba Usuário. */
  ...
  const footerActions = useMemo((): JiffySidePanelFooterActions => {
    if (state.tab === 'usuario') {
      ...
      return {
        ...
        // Mesmo submit do form — loading só no botão "Salvar" (submit)
  ...
  /** Aba Usuário indisponível: criação sem persistir perfil, ou perfil com alterações pendentes de Salvar. */
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col">
          {state.tab === 'perfil' ? (
            <NovoPerfilUsuario
              ref={perfilRef}
              key={`perfil-${state.perfilId ?? 'new'}-${state.mode}-sess-${formSession}`}
          ...
          {state.tab === 'usuario' ? (
            <NovoUsuario
              ref={usuarioRef}
              key={`usuario-${state.usuarioId ?? 'new'}-${state.usuarioId ? 'edit' : 'create'}-sess-${formSession}`}
        ...
        ? createPortal(
            ...
            >
              <div
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="perfis-usuarios-tabs-exit-title"
              >
                <h3
                  id="perfis-usuarios-tabs-exit-title"
